<template>
    <div>
        ...
        >
            ...
            <template v-slot:item.actions="{ item }">
                <v-btn
                    ...
                    small
                    color="success"
                    @click="confirm(item.id,item.idnumber)"
                >
                    Confirm
                </v-btn>
                <v-btn
        ...
        <v-dialog v-model="dialog" persistent max-width="300">
            <v-card>
                <v-card-title>Confirm {{idnumber}}?</v-card-title>
                <v-card-text>
                   <p> Do you want to confirm enrollment registration of {{idnumber}}?</p>
                </v-card-text>
                <v-card-actions>
...
export default {
    ...
    methods: {
        ...
        submit() {
             this.confirmEnrollment(this.id)
                .then((data) => {
                    this.$toast.success("Enrollment registration of "+this.idnumber+" was successfully confirmed. ", this.$toastOption);
                    this.setStepFrom(4)
                    this.setStepTo('')
