...
function initialState() {
    return {
        ...
        tableColumns: [
            {value: "subject.subcode", text: "Code",},
            {value: "subject.subdesc", text: "Description",},
            {value: "subject.subunit", text: "Unit",},
...
const actions = {
    fetchData({ commit, state }) {
        ...
        axios.get(route('api.admin.enrolled-subjects.index', state.serverQuery))
            .then(response => {
    ...
    fetchAllData({ commit, state }, params = {}) {
        commit('setLoading', true);
        axios.get(route('api.admin.enrolled-subjects.index', { ...params, perPage: 5000 }))
            .then(response => {
    ...
    destroyData({ dispatch, commit, state }, id) {
        axios.get(route('api.admin.enrolled-subject.destroy', id))
            .then(data => {
...
const mutations = {
    ...
    setAppID(state, value) {
        state.serverQuery.app_id = value
    },
    setYear(state, value) {
