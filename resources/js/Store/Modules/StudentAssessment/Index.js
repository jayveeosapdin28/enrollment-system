...
const actions = {
    fetchData({ commit, state }) {
        ...
        axios.get(route('api.admin.enrollment-application.assessment.index', state.serverQuery))
            .then(response => {
    ...
    fetchAllData({ commit, state }, params = {}) {
        commit('setLoading', true);
        axios.get(route('api.admin.enrollment-application.assessment.index', { ...params, perPage: 5000 }))
            .then(response => {
