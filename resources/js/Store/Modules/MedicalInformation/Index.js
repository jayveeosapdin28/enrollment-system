...
const getters = {
    ...
    medicalQuestions: state => state.medicalQuestions,
...
const actions = {
    fetchData({ commit, state }) {
        ...
        axios.get(route('api.student.contacts.index', state.serverQuery))
            .then(response => {
    ...
    fetchAllData({ commit, state }, params = {}) {
        commit('setLoading', true);
        axios.get(route('api.student.medical-information.index', { ...params, perPage: 5000 }))
            .then(response => {
    ...
    fetchMedicalQuestion({ commit, state }, params = {}) {
        commit('setLoading', true);
        axios.get(route('api.student.medical-questions.index', { ...params, perPage: 5000 }))
            .then(response => {
                commit('setMedicalQuestions', response.data.data)
            })
            .finally(function () {
                commit('setLoading', false);
            })
...
const mutations = {
    ...
    setMedicalQuestions(state, query) {
        state.medicalQuestions = query
    },
    resetState(state) {
