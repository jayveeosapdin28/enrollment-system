...
const actions = {
    fetchData({ commit, state }) {
        ...
        axios.get(route('api.student.enrollment-applications.index', state.serverQuery))
            .then(response => {
    ...
    fetchAllData({ commit, state }, params = {}) {
        commit('setLoading', true);
        axios.get(route('api.student.enrollment-applications.index', { ...params, perPage: 5000 }))
            .then(response => {
    ...
    destroyData({ dispatch, commit, state }, id) {
        return new Promise((resolve, reject) => {
            axios.get(route('api.student.enrollment-applications.destroy', id))
                .then(data => {
                    resolve(data)
